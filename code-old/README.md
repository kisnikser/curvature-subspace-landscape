...
From the **repository root** (imports assume `code-old` on `PYTHONPATH` via the scripts):
...
- **JSON** is written to `code-old/output/landscape/landscape_experiments.json` (path from `config.yaml` → `common.output_dir`).
- **Primary and ablation PDFs** are written to `paper/figures-old/` for inclusion in `main.tex`.

Optional: `python code-old/plot_scaling_from_json.py /path/to/custom.json`
...
If `data.text_path` is null, the runner downloads the requested corpus to `code-old/data/`.
