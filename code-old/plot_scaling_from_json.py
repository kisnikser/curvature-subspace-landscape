...
From repository root:
  python code-old/run_experiments.py
  python code-old/plot_scaling_from_json.py

PDFs are written to paper/figures-old/ (matches \\graphicspath in main.tex).

Optional: set env TIMES_NEW_ROMAN_TTF to the path of a Times New Roman .ttf, or place Times New Roman.ttf next to this script.
"""
...
# Larger than matplotlib defaults; axis labels larger still (see _apply_matplotlib_style).
_FONT_SIZE = 13
_TICK_LABEL_SIZE = 12
_AXIS_LABEL_SIZE = 17
_TITLE_SIZE = 14
...
def _register_times_from_env() -> str | None:
    """Register Times New Roman from TIMES_NEW_ROMAN_TTF (or MPL_TIMES_NEW_ROMAN_TTF) or Times New Roman.ttf next to this script."""
    candidates: list[Path] = []
    for key in ("TIMES_NEW_ROMAN_TTF", "MPL_TIMES_NEW_ROMAN_TTF"):
        raw = os.environ.get(key)
        if raw:
            candidates.append(Path(raw).expanduser())
    candidates.append(_SCRIPT_DIR / "Times New Roman.ttf")
...
_REGISTERED_TIMES_NAME = _register_times_from_env()


def _apply_matplotlib_style() -> None:
    serif = ["DejaVu Serif", "DejaVu Sans", "DejaVu Sans Mono"]
    if _REGISTERED_TIMES_NAME:
        serif = [_REGISTERED_TIMES_NAME, *serif]
    plt.rcParams.update(
        {
            "font.family": "serif",
            "font.serif": serif,
            ...
            "figure.titlesize": _TITLE_SIZE,
        }
    )


_apply_matplotlib_style()
_FIGURES = _REPO_ROOT / "paper" / "figures-old"
_CONFIG = _SCRIPT_DIR / "config.yaml"
...
def main() -> None:
    ...
    if not json_path.is_file():
        print(f"Missing {json_path}; run: python code-old/run_experiments.py", file=sys.stderr)
        raise SystemExit(1)
