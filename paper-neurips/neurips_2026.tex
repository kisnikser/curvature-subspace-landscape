...
However, neither criterion is placed in a unified framework, nor do they exploit the known curvature structure of the loss landscape.
...
Thus a stabilization criterion is determined jointly by the aggregation order $p$ and the probing law $q$, which selects which regions and directions of parameter space are emphasized.
...
We analyze $\Delta_2^{(D)}$ under a local quadratic model of the increment field $\mathcal L_{k+1}(\mathbf w)-\mathcal L_k(\mathbf w)$ near $\mathbf w_k^*$; all results are local guarantees rather than global statements.
