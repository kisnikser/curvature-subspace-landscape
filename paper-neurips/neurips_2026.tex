...
The experiments confirm that the Gaussian-moment estimator is accurate within the valid quadratic regime ($\sigma\le10^{-3}$) across all tested $D\in[1,100]$, and reveal that proxy accuracy degrades rapidly outside this regime.
...
\begin{itemize}
    ...
    \item We empirically validate three estimators---direct Monte Carlo, Quadratic MC, and the closed-form Gaussian-moment (GM) estimator---and show that GM achieves accuracy within $\sigma\le10^{-3}$ while being approximately $18{,}000\times$ faster than direct MC in the evaluation stage.
...
Within the valid proxy regime ($\sigma\le10^{-3}$), the relative error is small across all tested subspace dimensions, confirming that the GM estimator serves as an accurate and computationally efficient surrogate for Direct~MC in practice.
...
At the same time, quadratic proxy estimators are reliable only in an extremely local regime ($\sigma\le10^{-3}$), beyond which the GM estimator diverges from Direct~MC.
