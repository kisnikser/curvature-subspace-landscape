...
class RustBPETokenizer:
    ...
    def from_directory(cls, tokenizer_dir):
        ...
        with open(pickle_path, "rb") as f:
            enc = pickle.load(f)
        # Trained RustBPE uses <|bos|>; tiktoken pretrained (e.g. gpt2) uses <|endoftext|> as doc delimiter.
        for bos in ("<|bos|>", "<|endoftext|>"):
            try:
                return cls(enc, bos)
            except KeyError:
                continue
        raise KeyError(
            "Neither <|bos|> nor <|endoftext|> is a single token in this encoding; "
            "use a nanochat-trained tokenizer or scripts.bootstrap_pretrained_tokenizer with a compatible encoding."
        )
