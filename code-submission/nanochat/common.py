...
def get_base_dir():
    """Root dir for tokenizer, parquet shards, checkpoints, reports.

    Override with env ``NANOCHAT_BASE_DIR``. Default in this repo: ``code/.nanochat``
    (sibling of the ``nanochat/`` package), so caches stay inside the ``code`` tree.
    Upstream nanochat defaults to ``~/.cache/nanochat``; we prefer a repo-local path here.
    """
    if os.environ.get("NANOCHAT_BASE_DIR"):
        nanochat_dir = os.environ.get("NANOCHAT_BASE_DIR")
    else:
        _pkg_dir = os.path.dirname(os.path.abspath(__file__))  # .../code/nanochat
        _code_dir = os.path.dirname(_pkg_dir)  # .../code
        nanochat_dir = os.path.join(_code_dir, ".nanochat")
